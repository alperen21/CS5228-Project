...
def main():
    ...
    df_all_reviews = pd.concat(reviews)
    df_all_summaries = pd.concat(summaries)
    df_merged = pd.merge(df_all_reviews, df_all_summaries, on='app_id', how="inner")
    df_merged.to_csv(f"steam_{appid_num_to_get}_ids_{review_num_to_get}_summary_reviews.csv")
    end_time = time.time()
